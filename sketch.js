...
function draw(){
  ...
  if(stage!="welcome" && stage!="startTransition"){
    drawStopwatch()
  }
}
...
function drawToppings(){
  ...
  for(let k in usedCounts){
    if(!recipe.hasOwnProperty(k) && (usedCounts[k]||0)>0){
      allCorrect=false
      break
    }
  }
...
function drawCheese(){
  ...
  if(cheeseStrokePoints.length>0){
    // Draw uncooked pizza clipped to painted regions
    push()
    drawingContext.save()
    drawingContext.beginPath()
    for(let p of cheeseStrokePoints){
      drawingContext.arc(p.x,p.y,cheeseBrushRadius,0,TWO_PI)
    }
    drawingContext.clip()
...
function drawEnd(){
  ...
  strokeWeight(4)
  textSize(18)
  text("Presentation score",width/2,212)
  drawStars(width/2-90,238,presentationStars,5)
  text("Slicing score",width/2,292)
  drawStars(width/2-90,318,slicingStars,5)
  text("Overall score",width/2,372)
  drawStars(width/2-90,398,overallStars,5)

  textSize(14)
  let seconds=stopwatchElapsedMs/1000
  text("Time: "+nf(seconds,0,2)+"s  |  Overall cap from time: "+overallCapStars+" stars",width/2,442)
  noStroke()
...
function paintCheeseAt(x,y){
  ...
  for(let i=0;i<4;i++) cheeseParticles.push(new CheeseParticle(x,y))
...
function calculateScore(){
  ...
  let avgStars=(presentationStars+slicingStars)/2
  overallCapStars=maxOverallStarsFromTime(stopwatchElapsedMs/1000)
  overallStars=min(overallCapStars,constrain(round(avgStars),0,5))
}
...
function spawnConfetti(){ for(let i=0;i<300;i++) confetti.push(new Confetti()) }
